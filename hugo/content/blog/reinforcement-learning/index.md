...
This hypothesis seems to be further validated by OpenAI's GPT-5 release. GPT-5 is cheaper than GPT-4o while remaining the same speed (tokens/second or end-to-end latency), and massively cheaper and faster than GPT-4.5, Claude 4 Opus, or Grok 4, which are known to be among the largest models. This low cost and fast response indicates that GPT-5 is likely not much larger than GPT-4o, indicating that generative AI capabilities can continue to increase (presumably due to better training methods and more training time) without significantly scaling model sizes.[^gpt5]
...
In contrast to RLHF, RLVR _does not need human annotation or alignment_. Instead, it provides reward signals based on response correctness; this works best on domains with verifiably-correct answers, such as mathematics, code, or when rubrics can be used to unambiguously provide pass/fail signal, such as for instruction following precision. This makes RLVR useful for task-oriented training.

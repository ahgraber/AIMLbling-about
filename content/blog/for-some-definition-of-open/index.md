...
Specifically, the OSI notes that the preferred form of OSAI meets these 4 points of openness by including open access to training data, open source code used to train and run the system, and open model weights.[^osai] [^osai_faq]
...
Meta (and others) should not represent Llama (or open-weight) models as "open" without full openness for the training data, source code, model weights, and licenses.

Perhaps an alternative is to follow the recent trend(?) in the software world of making project "source available" (i.e., where the code is available but use is constrained by license).
...
admittedly, this "solution" does not protect the secret post-training recipes that really separate models as state-of-the-art and give them their character.

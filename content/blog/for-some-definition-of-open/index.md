...
is defined by training data, the source code required to train a model artifact, the (trained) model artifact, and the source code required to use the artifact
(whereas a traditional software product is solely defined by its source code).
As a result, the traditional definition of "open source", which is defined regarding the use, redistribution and derivations of the software source code[^oss] is insufficient to fully define "Open Source Artificial Intelligence" (OSAI).

## Open Source Artificial Intelligence

Fortunately for us, the Open Source Initiative released its formal definition of Open Source Artificial Intelligence at the end of October, 2024, stating:
...
Specifically noting that the preferred form that meets these 4 points of openness includes open access to training data, open source code used to train and run the system, and open model weights.[^osai] [^osai_faq]

The Linux Foundation has a similar definition, stating:
...
Open Core Ventures, a venture capitalist firm, considers the licensing differences required for AI/ML models.
...
These definitions of Open Source AI conflict with the typical use of "open" in the AI space, and the juxtaposition highlights the reason my brain twinges every time I hear "Open Source AI" --
most organizations falsely synonymize "open weight" models as "open source", while retaining license restrictions and closing source code as private intellectual property.
For instance, Meta touts the openness of its Llama-series models with terms stating
_"Our model and weights are licensed for researchers and commercial entities, upholding the principles of openness"_ but also specify use limitations in their license[^llama3];
...
Why do we see handwaving around "open"? If you believe the large AI labs, it's a necessary side effect of their research mission.
OpenAI and Anthropic are ostensibly research labs founded because of safety concerns with the inevitable development of AI/AGI.
...
Case in point, OpenAI stats in the `o1` reasoning model blog announcement:
...
with a governing body focused on the mission with a for-profit arm to finance it.
Anthropic is a Delaware Public Benefit Corporation, governed by a Long-Term Benefit Trust (LTBT).
In contrast to OpenAI, the LTBT governance organization is not nonprofit, but recognizes that a purely for-profit/capitalist
...
and also want to call out that they do publish research, such as [Scaling Monosemanticity: Extracting Interpretable Features from Claude 3 Sonnet](https://transformer-circuits.pub/2024/scaling-monosemanticity/index.html)
and [Introducing Contextual Retrieval \ Anthropic](https://www.anthropic.com/news/contextual-retrieval), that are instructive and useful in practice.
{{< /callout >}}
...
{{< callout type="info" >}}
I'm still not sure about the financial relationship between Ai2 and the AI2 Incubator.
...
I am not suggesting any foul play or under-the-table dealings... it would just not surprise me if there was a financial relationship given the costs of R&D in AI and that the nonprofit Ai2 reports no income streams other than donations.
{{< /callout >}}
...
However, when people think of "open" AI, they primarily orient themselves to Meta's open-weights Llama models.
...
Meta moved toward AI through a combination of factors - its stewardship of pytorch, its push to improve its recommendation systems, and in order to respond to TikTok.
...
If AI is "going to be more like people going from not having computers to having computers," then Meta wants to be the default,
so that AI applications are built on its platforms.
...
fully open-sourcing a foundation model would allow the research community to start experiments from the existing cutting edge,
...
Even if each of these labs fully open-sourced their AI systems, I don't it would lead to an explosion of new SOTA models because of the cost (and GPU availability) constraints.
...
It's not _whether_ models are open source or not, it's the "open-washing" and misrepresentation that bothers me - and that I think is a problem for the industry.
Anthropic have fully closed models - they do not even have an open tokenizer! That's fine.
Meta (and others) should not represent Llama models as "open" without full openness for the training data, source code, model weights, and licenses.

Perhaps the answer is to follow the recent trend(?) in the software world of making project source-available (i.e., where the code is available but use is constrained by license).
The availability of source code and data would be a boon to the AI community as discussed above, and the license would protect the use of models;
admittedly, this "solution" does not protect the secret post-training recipes that really separate SOTA models and given them their character.

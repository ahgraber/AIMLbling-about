...
This is, admittedly, a toy experiment, since I'm running this on my own dime and on a limited set of source information.
However, running this has been extremely instructive on configuring valid experiments
...
I will be evaluating 4 different models - OpenAI's `GPT-4o-mini`, Anthropic's `Claude-3-Haiku`, `Llama-3.1-Instruct-70B-Turbo`, quantized and hosted by TogetherAI,
and a `Q4_K_M` quantized version of `Mistral-Nemo-Instruct-2407` that I self-host with [LMStudio](https://lmstudio.ai/).
I'll be using an embedding model from the same provider as the model where possible -
OpenAI's `text-embedding-3-small`, Anthropic's recommended embedding provider VoyageAI's `voyage-3-lite` [^voyage], TogetherAI's `m2-bert-80M-8k-retrieval`, and Nomic's `nomic-embed-text-v1.5`.

The dataset that I will create a RAG over is comprises the articles from this blog (posted between April and September, 2024), and the self-documentation markdown files from [my homelab gitops repo](https://github.com/ahgraber/homelab-gitops-k3s).

The procedure to build a knowledgegraph and test set is better described in [part two of the series]({{< ref "/blog/ragas-to-riches-2" >}}).
The knowledge graph is built with `Mistral-Nemo-Instruct-2407` and `nomic-embed-text-v1.5`.
Based on the knowledge graph, I generate ~100 (it ended up being 112) test questions each per model, ending up with dataset of 448 questions.

I also create a different RAG index per model/embedding pair. Documents from the knowledge base are chunked based on markdown sections, and embedded into vector space.
Note that this is a different chunking and embedding process from the knowledge graph used for testset generation, which ensures that the testset is independent of the RAG architecture we might be evaluating!

I generate baseline responses, retrievals, and RAG responses for each of the 448 questions with each model/retriever.
The baseline response is simply what the model responds to the test question _without_ providing it any retrieved context, meaning the answer must come from internal knowledge - a "closed-book exam".  
Baseline response quality is evaluated by analyzing `semantic similarity` to the "ground truth" reference and `answer relevance` to the test question.  
Retrievals query each RAG index (OpenAI, Anthropic/Voyager, Together, local) for each question in the test set.
Retrievals are evaluated by analyzing `context recall` and `context precision` with respect to the "ground truth" context from the test set.  
RAG responses takes the retrievals from each RAG index, and provides both the retrievals as additional context and the test question to the LLM generating the answer in "open-book exam" fashion.
RAG response quality is evaluated by analyzing `semantic similarity` to the "ground truth" reference, `answer relevance` to the test question, and `faithfulness` to the retrieved context.
...
Given my hypothesis - that models will prefer their own responses, every evaluation described above is actually run 4x - once for each LLM/embedding model pair.
For example, I might generate baseline response, retrievals, and RAG response with OpenAI for a question in the test set.
To evaluate the baseline response from OpenAI, I measure `answer relevance` to the test question with each of 4 models (`GPT-4o-mini`,`Claude-3-Haiku`, `Llama-3.1-Instruct-70B-Turbo`, and `Mistral-Nemo-Instruct-2407`).
To evaluate the context retrievals, I measure `context precision` and `context recall` again using all 4 models for each metric.
Finally, I follow the same procedure for RAG response, evaluating `answer relevance` and `faithfulness` both using all 4 models.

> I cheated a bit and only used the self-hosted `nomic-embed-text-v1.5` to evaluate `semantic similarity` for the baseline and RAG response cases
> because I'm primarily concerned about LLM-as-a-Judge performance (not embedding model performance) for this experiment.
...
Model performance on baseline `answer_relevance` seems more related to model release recency than size,
...
When looking at evaluator performance, `Claude-3-Haiku` is again the most optimistic model.

{{< tabs items="Context Precision,Context Recall,Side Quest" >}}
{{< tab >}}
...
Over both Semantic Similarity and Answer Relevance, we can see both the improvement that RAG provides we see the damage that a poor retriever can do.
...
When using `nomic-embed-text-v1.5` to assess how relevant (similar) baseline responses are to ground-truth reference, both the improvement that RAG provides we see the damage that a poor retriever can do.
...
- analyse evaluator alignment (correlation)
